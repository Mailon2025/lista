...
const CACHE_KEY = 'series_poster_cache_v4';
const CACHE_DURATION = 7 * 24 * 60 * 60 * 1000; // 7 days
const FAILED_CACHE_DURATION = 24 * 60 * 60 * 1000; // 1 day para falhas

// API Key do TMDB
const TMDB_API_KEY = '09bf5248a7b6a0818c84d9f61858427a';
...
const RATE_LIMIT_DELAY = 300; // TMDB permite ~40 req/10s
...
function getCached(cleanName: string): { found: boolean; url: string | null } {
  ...
  if (!cached) return { found: false, url: null };
...
export function clearPosterCache(): void {
  ...
  localStorage.removeItem('series_poster_cache');
  localStorage.removeItem('series_poster_cache_v2');
  localStorage.removeItem('series_poster_cache_v3');
  console.log('✅ Cache de posters limpo! Recarregue a página.');
}
...
// Expõe funções no window para debug via console
if (typeof window !== 'undefined') {
  (window as any).clearPosterCache = clearPosterCache;
